...
function refreshTranscriptEvidence(projectData, transcript) {
  const transcriptId = String(transcript && transcript.id || '');
  if (!transcriptId) return;
  const transcriptTitle = String(transcript.title || '');
  const date = String(transcript.date || transcript.uploadedAt || '');
  (projectData.stories || []).forEach(story => {
    let changed = false;
    story.updates = (Array.isArray(story.updates) ? story.updates : []).map(update => {
      let updateChanged = false;
      const refs = normalizeSourceRefs(update).map(ref => {
        if (ref.transcriptId !== transcriptId) return ref;
        const sourceFollowedTitle = !ref.source || ref.source === ref.transcriptTitle;
        updateChanged = true;
        changed = true;
        return {
          ...ref,
          transcriptTitle,
          date,
          source: sourceFollowedTitle ? transcriptTitle : ref.source
        };
      });
      if (!updateChanged) return update;
      update.sourceRefs = refs;
      return synchronizeUpdate(update);
    });
    if (changed) recalculateStoryDerivedFields(story, { clearWhenEmpty: true });
  });
}
