...
function buildGroundedSelectionRequest({ task, records, maxClaims = 12 }) {
  ...
  const system = [
    ...
    'Return JSON only using this schema: {"claims":[{"sourceId":"existing sourceId","excerpt":"one complete verbatim sentence, or the complete field value when it has no sentence punctuation"}]}.',
    `Return at most ${boundedMaxClaims} claims. Preserve negation and qualifiers. Do not paraphrase, infer, or add unsupported statements.`
  ].join(' ');
...
function validateGroundedClaims(rawOutput, records, options = {}) {
  ...
  return parsed.claims.map((claim, index) => {
    ...
    if (!completeSourceSpans(source.text).includes(excerpt)) {
      throw new Error(`AI claim ${index + 1} must preserve a complete source sentence or field value.`);
    }
