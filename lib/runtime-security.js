...
async function fetchWithTimeout(url, options, {
  ...
  consumeResponse,
  responseErrorMessage = 'The AI provider returned an unreadable response.'
} = {}) {
  if (typeof consumeResponse !== 'function') {
    throw new ProviderRequestError('A bounded AI response consumer is required.');
  }
  ...
  try {
    ...
    if (responseReceived) throw new ProviderRequestError(responseErrorMessage);
